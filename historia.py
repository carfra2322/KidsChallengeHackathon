...
def new_history():
	...
	.reprompt("Sorry I didn't get your made up name. Please tell me your made-up name.")
...
@ask.intent("gender_intent", convert={'gender':str})
...
def story_selectionFunction(story):
	...
	return question(fmsg).reprompt("Sorry I didn't get that. What did the old man grab ? a Spoon or a butterknife?")

@ask.intent("spooky_utensil", convert={'spookyutensil':str})
def spooky_utensil(spookyutensil):
	py_spooky_utensil = format(spookyutensil)
	if py_spooky_utensil == "spoon":
		fmsg = render_template('spooky_spoon')
	else :
		fmsg = render_template('spooky_butterknife')
	session.attributes['spookyutensil'] = py_spooky_utensil
	return question(fmsg).reprompt("Please select another story")
